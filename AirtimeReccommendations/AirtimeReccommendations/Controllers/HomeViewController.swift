    ...
    func writeToCSV(video: Video) {
        ...
        while let row = csv.next() {
            ...
            if video.items[0].contentDetails.duration.contains("M") {
                var split = video.items[0].contentDetails.duration.components(separatedBy: "M")
                let minutes = split[0].components(separatedBy: "T")[1]
                let seconds = split[1][0]
                let duration = (Int(row[4]) ?? 600) / ((Int(minutes) ?? 10 * 60) + seconds.wholeNumberValue!)
                try! outputCSV.write(row: [row[0], row[1], row[2], row[3], row[4], String(duration)])
            } else if video.items[0].contentDetails.duration.contains("S") {
